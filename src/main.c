...
void setup(void) {
    ...
    load_mesh("../assets/obj/runway.obj", "../assets/textures/runway.png", vec3_new(1, 1, 1), vec3_new(0, -1.5, +23), vec3_new(0, 0, 0));
    load_mesh("../assets/obj/f22.obj", "../assets/textures/f22.png", vec3_new(1, 1, 1), vec3_new(0, -1.3, +5), vec3_new(0, -M_PI / 2, 0));
    load_mesh("../assets/obj/efa.obj", "../assets/textures/efa.png", vec3_new(1, 1, 1), vec3_new(-2, -1.3, +9), vec3_new(0, -M_PI / 2, 0));
    load_mesh("../assets/obj/f117.obj", "../assets/textures/f117.png", vec3_new(1, 1, 1), vec3_new(+2, -1.3, +9), vec3_new(0, -M_PI / 2, 0));
}
    ...
    while (SDL_PollEvent(&event)) 
    {
        ...
        case SDL_KEYDOWN:
            ...
            if (event.key.keysym.sym == SDLK_DOWN) {
				update_camera_direction(vec3_mul(get_camera_direction(), 5.0 * delta_time));
...
void process_graphics_pipeline_stages(mesh_t* mesh) {
    ...
    for (int i = 0; i < num_faces; i++) {
        ...
        // Calculate the triangle face normal to perform backface culling
...
void update(void) {
    ...
    if (fps_update_timer >= 1.0f) {  // Update every second
        current_fps = (int)(1.0f / delta_time);
...
void render(void) {
    ...
    for (int i = 0; i < num_triangles_to_render; i++) {
        ...
        if (should_render_wireframe_vertex()) {
    ...
    draw_text(get_window_width() - 80, 10, 0xFFFFFFFF, fps_text);
